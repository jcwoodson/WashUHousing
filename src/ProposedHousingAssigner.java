...
public class ProposedHousingAssigner {
	...
	public static void main(String[] args) throws IOException {
		for (int z = 0; z < 1000; z++) {
			...
			boolean[] individual = new boolean[NUM_STUDENTS];
			...
			for (int i = 0; i < NUM_STUDENTS-NUM_STUDENTS/300*100; i++) {
				int topRanked = -1;
			...
			for (int leader : g) {
				...
				for (int i = 0; i < NUM_TYPES; i++) {
					...
					if (ROUND_TWO[room] && SUITE_SIZES[room] == size && spaceAvailable[room] > 1 && spaceAvailable[room] >= size) {
						...
						roomFound = true;
						break;
					} else if (size == 1 && ROUND_TWO[room] && spaceAvailable[room] > 1 && i < 5) {
						roomFound = true;
						spaceAvailable[room]--;
						rooms[leader] = room;
						peopleAssigned.get(room).add(leader);
						break;
					}
				}
				if (roomFound) {
			...
			for (int i = 0; i < NUM_TYPES; i++) {
				while (peopleAssigned.get(i).size()/SUITE_SIZES[i]*SUITE_SIZES[i] != peopleAssigned.get(i).size()) {
					int p = peopleAssigned.get(i).remove(peopleAssigned.get(i).size()/SUITE_SIZES[i]*SUITE_SIZES[i]);
					spaceAvailable[i]++;
					inGroup[p] = false;
				}
			...
			peopleAssigned = new ArrayList<ArrayList<Integer>>();
			for (int i = 0; i < NUM_TYPES; i++) {
				peopleAssigned.add(new ArrayList<Integer>());
			}
			for (int leader : g) {
			...
			for (int i = 0; i < NUM_TYPES; i++) {
				...
				for (int j = NUM_STUDENTS-1; j >= NUM_STUDENTS-NUM_STUDENTS/300*100; j--) {
					if (SUITE_SIZES[rankings[j][i]] == 1)
						continue;
					if (spacesLeftThree == 0)
						break;
					if (!inGroup[j] && !ROUND_TWO[rankings[j][i]] && spaceAvailable[rankings[j][i]] > 0 && studentsLeft >= SUITE_SIZES[rankings[j][i]]) {
						...
						for (int m = 0; m < NUM_TYPES; m++) {
							if (spacesLeftSuite == 0)
								break;
							for (int n = NUM_STUDENTS-1; n >= 0; n--) {
								if (spacesLeftSuite == 0)
									break;
			...
			BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("washu/proposed-"+String.format("%03d",z)+".csv"), "utf-8"));
