...
public class UtilityCalculator {
	...
	public static void main(String[] args) throws IOException {
		for (int z = 0; z < 1000; z++) {
			...
			double[][] place = new double[3][NUM_STUDENTS];
			double[][] people = new double[3][NUM_STUDENTS];
			...
			for (int i = 0; i < NUM_STUDENTS; i++) {
				...
				if (SUITE_SIZES[rooms[i]] > 1) {
					...
					people[0][i] = (double)tempBig/div/PERSON_AVG;
				}
			}
			int numPeople = 0;
			double dist = 0;
			...
			for (int i = 0; i < NUM_STUDENTS; i++) {
				if (SUITE_SIZES[rooms[i]] == 1) {
					numPeople++;
					people[0][i] = dist/numPeople;
				}
			}
			
			sc = new Scanner(new BufferedReader(new InputStreamReader(new FileInputStream("washu/current-"+String.format("%03d",z)+".csv"))));
			for (int i = 0; i < NUM_STUDENTS; i++) {
				String[] line = sc.nextLine().split(",");
				int j = 0;
				j++;
			...
			for (int i = 0; i < NUM_STUDENTS; i++) {
				...
				if (SUITE_SIZES[rooms[i]] > 1) {
					...
					people[1][i] = (double)tempBig/div/PERSON_AVG;
				}
			}
			numPeople = 0;
			dist = 0;
			...
			for (int i = 0; i < NUM_STUDENTS; i++) {
				if (SUITE_SIZES[rooms[i]] == 1) {
					numPeople++;
					people[1][i] = dist/numPeople;
				}
			}
			
			sc = new Scanner(new BufferedReader(new InputStreamReader(new FileInputStream("washu/proposed-"+String.format("%03d",z)+".csv"))));
			for (int i = 0; i < NUM_STUDENTS; i++) {
				String[] line = sc.nextLine().split(",");
				int j = 0;
				j++;
			...
			for (int i = 0; i < NUM_STUDENTS; i++) {
				...
				if (SUITE_SIZES[rooms[i]] > 1) {
					...
					people[2][i] = (double)tempBig/div/PERSON_AVG;
				}
			}
			numPeople = 0;
			dist = 0;
			...
			for (int i = 0; i < NUM_STUDENTS; i++) {
				...
				for (int j = 0; j < 3; j++) {
					output.append(place[j][i]*100+",");
					output.append(people[j][i]*100+",");
					output.append((place[j][i]*prefRoom[i]+people[j][i]*(100-prefRoom[i]))+",");
				}
				output.deleteCharAt(output.length()-1);
				output.append("\n");
			}
			
			BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("washu/utility-"+String.format("%03d",z)+".csv"), "utf-8"));
